...
function createMGRSGrid(extent: Extent, resolution: number): Feature<LineString>[] {
  const features: Feature<LineString>[] = [];
  const [minLon, minLat, maxLon, maxLat] = extent.map(coord => toLonLat(coord, 'EPSG:3857'));
  
  // Convert extent corners to MGRS
  const bottomLeft: MGRSCoordinate = mgrs.forward([minLon, minLat]);
  const topRight: MGRSCoordinate = mgrs.forward([maxLon, maxLat]);

  // Generate grid lines (100km grid)
  const gridSize: number = 100000; // 100km in meters
...
map.on('moveend', () => {
  ...
  const gridSource = map.getLayers().getArray()[1].getSource() as VectorSource;
